...
############################################################
class Hamiltonian:
    def __init__(self, system_params, combined_rates, mode, kappa):
        self.system_params = system_params
        self.combined_rates = combined_rates
        self.mode = mode
        self.kappa = kappa
        self.mode = mode
    ...
    def calculate_shift(self):
        if self.mode == 'polaron':
                ...
                # integrand = self.combined_rates.J_vib(w) / w # * (1 / np.tanh(beta_vib * w / 2))
                # shift = float(np.trapz(integrand, w))
                omega = np.linspace(1.0e-4, 2000, 10000)
                integrand = np.array([self.combined_rates.J_vib(w) / w for w in omega]) # * (1 / np.tanh(beta_vib * w / 2))
    ...
    def get_hamiltonian(self):
        ...
        omega_1 = self.system_params.omega_q1 - shift
        omega_2 = self.system_params.omega_q2 - shift

        J_scaled = self.system_params.J * (self.kappa**2)

        H = omega_1 * np.dot(sp1, sm1) + omega_2 * np.dot(sp2, sm2) + (J_scaled / 2) * (np.dot(sp1, sm2) + np.dot(sm1, sp2))
        return H
