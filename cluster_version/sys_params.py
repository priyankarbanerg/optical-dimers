...
class Constants:
  ...
  def __init__(self):
    ...
    self.k_B_ps = self.k_B / self.hbar_ps  # ps / K
...
class DipoleParameters:
    ...
    def __init__(self, config_name, data_dir="files/dipole_positions/", new_dir=None, average=False):
        if config_name not in ["H", "J", "ortho", "oppo"]:
        ...
        self.new_dir = new_dir  # disorder_angles should be a tuple (theta, phi)
    ...
    def calculate_dipole_properties(self, pos1, pos2, dir1, dir2):
        ...
        dir1 = np.array([dir1[0], dir1[1], dir1[2]])
    ...
    def get_dipole_parameters(self):
        ...
        # Initialize lists to store dipole properties
        r_hat = np.array([])
        d1_hat = np.array([])
        d2_hat = np.array([])
        ...
        for i in range(0, len(dipole_positions), 2):
            ...
            if self.average and self.new_dir is not None:
                # Introduce disorder using two spherical angles (theta, phi)
                d2_h = self.new_dir
                # print(d2_h)
            # # Apply tilt based on spherical angles
            # d2_h = self.apply_tilt_to_dipole(d2_h, theta, phi)
        ...
        # d_hat = np.array([d1_hat, d1_hat, d2_hat, d2_hat])

        return r_hat, d1_hat, d2_hat #, d_hat
...
class SystemParameters(Constants):
    ...
    def calculate_parameters(self):
        ...
        # self.T_vib = 1.0 * 300.0               # Temperature of vibrational bath (K)


        self.gamma_p_1 = 1.0 * self.gamma_q1   # Incoherent Optical Pumping
        self.gamma_p_2 = 1.0 * self.gamma_q2   # Incoherent Optical Pumping
