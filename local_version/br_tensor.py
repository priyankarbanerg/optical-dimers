...
class BlochRedfieldCalculator:
    def __init__(self, H, a_ops_opt, Gamma_opt, rates, a_ops_vib=None, Gamma_vib=None, a_ops_coup=None, Gamma_coup=None, P_vib=None, mode=None, pump = None):
        self.H = H
    ...
    #         # print(np.ndenumerate(Gamma_coup))
    #         G[i, j, :] = np.array([G_ab(w) for w in trans_freq])
    #     return G
    ...
    #     for (i, j), G_ab in np.ndenumerate(Gamma_coup):
    #         G[i, j, :] = np.vectorize(G_ab)(trans_freq)  # Applies G_ab to each w in trans_freq efficiently
    ...
    def mathcal_F(self, k_q_r, alpha, beta, threshold=1e-8):
        ...
        mathcal_F[np.abs(k_q_r) < threshold] = np.dot(system_params.d1_hat, system_params.d2_hat)
    ...
    def BR_tensor_opt(self):
        ...
        for x in range(len(a_ops_S_opt)):
            ...
            for y in range(len(a_ops_S_opt)):
                ...
                k_q = tf / system_params.c_ps

                alpha = (np.dot(system_params.d1_hat, system_params.d2_hat) - (np.dot(system_params.d1_hat, system_params.r_hat)) * (np.dot(system_params.d2_hat, system_params.r_hat)))
                beta = (np.dot(system_params.d1_hat, system_params.d2_hat) - 3 * (np.dot(system_params.d1_hat, system_params.r_hat)) * (np.dot(system_params.d2_hat, system_params.r_hat)))

                k_q_r = k_q * system_params.r + 1e-16
    ...
    def total_BR_tensor(self):
        if mode == 'weak' and self.a_ops_vib is None:
...
#                         - np.kron(np.multiply(np.transpose(G_ab), A_a), np.transpose(A_b))
#                         + np.kron(np.eye(dim, dim), np.transpose(np.dot(np.multiply(A_a, G_ab), A_b)))
#                         - np.kron(A_a, np.transpose(np.multiply(A_b, G_ab)))) * np.dot(d_hat[x], d2_hat) * P_ab
...
#                         - np.kron(np.multiply(np.transpose(G_ab), A_a), np.transpose(A_b))
#                         + np.kron(np.eye(dim, dim), np.transpose(np.dot(np.multiply(A_a, G_ab), A_b)))
#                         - np.kron(A_a, np.transpose(np.multiply(A_b, G_ab)))) * np.dot(d_hat[x], d2_hat)
